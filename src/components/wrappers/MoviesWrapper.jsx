  ...
  return (
    <div className="mx-4 flex justify-center">
      ...
      style={{ maxWidth: 'calc(15% - 1rem)' }} title={`${movie.title}`}>
        ...
        <p className="my-1 sm:text-md text-sm sm:font-normal font-extralight text-center text-wrap break-word">Rating: {movie.vote_average}</p>
        <p className="my-1 sm:text-md text-sm sm:font-normal font-extralight text-center text-wrap break-word">Original Language: {movie.original_language}</p>
        <p className="my-1 text-center text-wrap break-word">Release Date: {movie.release_date}</p>
    </Link>
        ))}
      </div>
    </div>
